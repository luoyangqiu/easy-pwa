...
function entryFile(file) {
  ...
  fs.readFile(filePath, 'utf-8', function (err, data) {
    ...
    if (!htmlText.includes('<script src="/' + config.entryScript + '"></script>')) {
      // 判断是否己经存在注册入口文件
      var replaceText = '<script src="/' + config.entryScript + '"></script></body>';
    ...
    if (!newHTML.includes('<link rel="manifest" href="/manifest.json">')) {
      ...
      var _replaceText = '<link rel="manifest" href="/manifest.json">\n      </head>';
...
function createManifestFile() {
  // 创建manifest.json文件
  var targetPath = config.relativeFilePath + config.iconsPath;
  var paths = config.relativePath + "/" + targetPath;
  fs.access(paths, function (err) {
    // 创建文件夹是否存在
    if (err) {
...
function createServiceWorkerFile(data) {
  // 创建Service Worker文件
  var serviceWorkerFileName = config.relativeFilePath + 'sw.js';
  var entryScript = config.relativeFilePath + config.entryScript;
  ...
  var path = __dirname; // 目前配置是当前路径，后期需要增加自定义路径功能，预留路径判断功能
  // terser 
  var options = {
    toplevel: true,
    compress: {
